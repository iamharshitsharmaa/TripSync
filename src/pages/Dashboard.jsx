...
import { Plus, MapPin, Calendar, ChevronRight, Loader2 } from 'lucide-react'
...
function TripCard({ trip }) {
  // BUG FIX 1: was .('en', ...) — missing toLocaleDateString
  ...
  return (
    <Link
      to={`/trips/${trip._id}`}
      className="group block bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden
        hover:border-gray-600 hover:shadow-xl hover:shadow-black/40 transition-all duration-200"
    >
      {/* Cover strip */}
      <div className="h-28 bg-gradient-to-br from-blue-900/40 via-purple-900/30 to-gray-900 relative overflow-hidden">
        {trip.coverImage
          ? <img src={trip.coverImage} alt="" className="w-full h-full object-cover opacity-60" />
          : <div className="absolute inset-0 flex items-center justify-center text-5xl opacity-20">🌍</div>
        }
        <div className="absolute top-3 right-3">
          <span className={`text-xs font-semibold px-2 py-1 rounded-md ${statusColor[trip.status] || statusColor.active}`}>
            {trip.status}
          </span>
        </div>
      </div>
      ...
      <div className="p-5">
        <h3 className="font-bold text-white text-base mb-1 group-hover:text-blue-300 transition">
          {trip.title}
        </h3>
        {trip.description && (
          <p className="text-gray-500 text-xs mb-3 line-clamp-1">{trip.description}</p>
        )}

        <div className="flex items-center gap-3 text-xs text-gray-400 mb-4">
          <span className="flex items-center gap-1">
            <Calendar size={11} /> {start} – {end}
          </span>
          <span className="flex items-center gap-1">
            <MapPin size={11} /> {daysCount} day{daysCount !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex -space-x-2">
            ...
            {trip.members?.length > 4 && (
              <div className="w-7 h-7 rounded-full bg-gray-700 border-2 border-gray-900
...
function CreateTripModal({ onClose }) {
  ...
  const [form, setForm] = useState({
    title: '', description: '', startDate: '', endDate: '', currency: 'USD', budgetLimit: ''
  })
  ...
  const handleSubmit = (e) => {
    ...
    // BUG FIX 2: convert dates to ISO strings so Mongoose parses them correctly
    mutate({
      title:       form.title,
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-bold text-white">Create New Trip ✈️</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              placeholder="e.g. Jaipur Trip 2026"
              required
              className={inputCls}
              value={form.title}
              onChange={set('title')}
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white
            font-semibold text-sm rounded-xl transition shadow-lg shadow-blue-600/20"
        >
      ...
      {!isLoading && trips.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-gray-400 text-sm mb-6">Create your first trip to get started</p>
          <button
            onClick={() => setShowCreate(true)}
